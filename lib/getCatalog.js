require("dotenv").config();
const { MovieDb } = require("moviedb-promise");
const moviedb = new MovieDb(process.env.TMDB_API);
const { getGenreList } = require("./getGenreList");
const { getLanguages } = require("./getLanguages");
const { getTrending } = require("./getTrending");

async function getCatalog(type, id, language, genre, page) {
  if (id === "tmdb.top" && !genre) {return await getTrending(type, id, language, "week", page)}
  var parameters = {
    language: language,
    page: page,
  };
  const genre_id = await getGenreList(language, type);
  const languages = await getLanguages();
  if (type === "movie") {
    if (id === "tmdb.top") {
      let gen_name = ''
      if (genre) {
        gen_name = genre
          ? (genre_id.find((x) => x.name === genre) || {}).id || ''
          : genre;
        if (!gen_name) return Promise.reject(Error(`Could not find genre: ${genre}`))
      }
      parameters.with_genres = gen_name
    } else if (id === "tmdb.year") {
      parameters.primary_release_year = genre
    } else if (id === "tmdb.language") {
      parameters.with_original_language = languages
        .find((lang) => lang.name === genre)
        .iso_639_1.split("-")[0]
    }
    return await moviedb
      .discoverMovie(parameters)
      .then(async (res) => {
        const metas = res.results.map((el) => {
          return {
            id: `tmdb:${el.id}`,
            name: el.title,
            genre: el.genre_ids.map(genre => genre_id.find((x) => x.id === genre).name),
            poster: `https://image.tmdb.org/t/p/w500${el.poster_path}`,
            background: `https://image.tmdb.org/t/p/original${el.backdrop_path}`,
            posterShape: "regular",
            imdbRating: el.vote_average.toFixed(1),
            year: el.release_date ? el.release_date.substr(0, 4) : "",
            type: type,
            description: el.overview,
          }
        })
        return { metas };
      })
      .catch(console.error);
  } else {
    if (id === "tmdb.top") {
      parameters.watch_region = language.split('-')[1]
      parameters.with_watch_monetization_types = "flatrate|free|ads|rent|buy"
      let gen_name = ''
      if (genre) {
        gen_name = genre
          ? (genre_id.find((x) => x.name === genre) || {}).id || ''
          : genre;
        if (!gen_name) return Promise.reject(Error(`Could not find genre: ${genre}`))
      }
      parameters.with_genres = gen_name
    } else if (id === "tmdb.year") {
      parameters.first_air_date_year = genre
    } else if (id === "tmdb.language") {
      parameters.with_original_language = languages
        .find((lang) => lang.name === genre)
        .iso_639_1.split("-")[0]
    }
    return await moviedb
      .discoverTv(parameters)
      .then(async (res) => {
        const metas = res.results.map((el) => {
          return {
            id: `tmdb:${el.id}`,
            name: el.name,
            genre: el.genre_ids.map(genre => genre_id.find((x) => x.id === genre).name),
            poster: `https://image.tmdb.org/t/p/w500${el.poster_path}`,
            background: `https://image.tmdb.org/t/p/original${el.backdrop_path}`,
            posterShape: "regular",
            imdbRating: el.vote_average.toFixed(1),
            year: el.first_air_date ? el.first_air_date.substr(0, 4) : "",
            type: type,
            description: el.overview,
          }
        })
        return { metas };
      })
      .catch(console.error);
  }
}

module.exports = { getCatalog };
const axios = require("axios");

async function getDoctorWhoEpisodes(language) {
  try {
    const response = await axios.get(
      `https://api.themoviedb.org/3/tv/57243?api_key=6fe4174e9530b9fdf8a8dfbec274c283&language=${language}&append_to_response=seasons`
    );

    const { seasons } = response.data;

    const episodes = seasons.map((season) =>
      season.episodes.map((episode) => ({
        id: `tmdb:${episode.id}`,
        type: "series",
        name: episode.name,
        poster: `https://image.tmdb.org/t/p/original${episode.still_path}`,
        posterShape: "regular",
        releaseInfo: episode.air_date,
        genres: ["Sci-Fi & Fantasy", "Action & Adventure", "Drama"],
        description: episode.overview,
      }))
    );

    // Flatten the episodes array
    return episodes.flat();
  } catch (error) {
    console.error("Error fetching Doctor Who episodes:", error);
    return [];
  }
}

module.exports = { getDoctorWhoEpisodes };
const { getDoctorWhoEpisodes } = require("./getDoctorWhoEpisodes");

async function getCatalog(type, id, language, genre, page) {
  if (id === "tmdb.popular") {
    // Return popular content from TMDB
    // Implementation for popular content
  } else if (id === "tmdb.trending") {
    // Return trending content from TMDB
    // Implementation for trending content
  } else if (id === "tmdb.search") {
    // Return search results from TMDB
    // Implementation for search results
  } else if (id === "doctorwho.episodes") {
    // Return Doctor Who episodes
    return await getDoctorWhoEpisodes(language);
  } else {
    // Return content for specific category from TMDB
    // Implementation for specific category content
  }
}

module.exports = { getCatalog };
